...
void do_insertion_sort(int* a, int size)
{
...
void do_selection_sort(int* a, int s)
{
...
void do_quick_sort(int* a, int _l,int _r)
{
	int pivot=a[(_l+_r)/2];
	int l=_l;
	int r=_r;
	...
	if(l < _r) 
		do_quick_sort(a,l,_r);
	if ( _l < r)
		do_quick_sort(a,_l,r);
}
...
void do_heap_sort(int*a, int s)
{
	int last_parent=heap_parent(s-1);
...
void heapify(int*a, int root, int e)
{
	int lt=heap_lt(root);
	int rt=heap_rt(root);
...
int heap_lt(int parent)
{
	return 2*parent +1;
}

int heap_rt(int parent)
{
	return 2*parent +2;
}
...
void do_shell_sort(int* a, int s)
{
